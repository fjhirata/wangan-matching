...
const API = ""; // 同一オリジン（server.py が配信）
const $app = document.getElementById("app");
const TSUBO = 3.30578;

const state = {
  config: null, axes: [], scale: [], questions: [], mansions: [],
  screen: "intro",
...
async function boot() {
  try {
    const [cfg, qdoc, mans] = await Promise.all([
      fetch(API + "/api/config").then((r) => r.json()),
      fetch(API + "/api/questions").then((r) => r.json()),
      fetch(API + "/api/mansions").then((r) => r.json()),
    ]);
    state.config = cfg;
    ...
    const meta = cfg.dataMeta || {};
    const fresh = document.getElementById("dataFresh");
    if (fresh && meta.generatedAt) {
      fresh.textContent = `データ更新 ${meta.generatedAt.slice(0, 10)}／坪単価 基準月 ${meta.priceAnchor || ""}`;
    }
    ...
    document.getElementById("boot").innerHTML =
      ...
      '<p style="margin-top:14px;line-height:1.7">APIに接続できませんでした。<br>ターミナルで<br><code>python3 backend/server.py</code><br>を実行し、<br><b>http://localhost:8000</b> を開いてください。</p></div>';
    console.error(e);
  }
}
...
async function submit() {
  ...
  try {
    const res = await fetch(API + "/api/diagnose", {
      ...
      body: JSON.stringify({
        age: state.input.age, income: state.input.income,
        areaSqm: state.input.area, answers: state.answers,
      }),
    }).then((r) => r.json());
    state.result = res;
